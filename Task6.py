...
Будем использовать модуль requests для запроса и отображения кратких данных о погодев выбранном городе из открытых источников.
...
def parse_weather_output(data):
    '''Функция принимает на вход словарь c ответом от погодного сервиса и возвращает словарь с ключами:
...
def parse_user_input():
    ...
    cписок словарей с данными о погоде в каждом городе. Перечень ключей каждого словаря:
    Город, Текущая температура, Ощущаемая температура, Погодные условия, Ветер (м/с)
    '''
